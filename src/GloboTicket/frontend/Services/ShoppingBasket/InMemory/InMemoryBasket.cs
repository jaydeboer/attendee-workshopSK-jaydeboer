...
namespace GloboTicket.Frontend.Services
{
    class InMemoryBasket
    {
        ...
        public void Remove(Guid lineId)
        {
            ...
            if (index >= 0) 
            {
                // Store the item for logging before removing it
                var itemForLogging = Lines[index];
                Lines.RemoveAt(index);
                
                // Now safely log the removed item details
                var logMessage = $"Removed item: {itemForLogging.Event.Name}";
                // Log message could be sent to a logging framework here
            }
        }
        ...
        {
            decimal totalPrice = CalculateSubtotal(applyPromoCode);
        ...
        /// <summary>
        /// Calculates the subtotal price without administration costs
        /// </summary>
        /// <param name="applyPromoCode">Whether to apply promo code discounts</param>
        /// <returns>The subtotal without administration costs</returns>
        public decimal CalculateSubtotal(bool applyPromoCode = false)
        {
            ...
            foreach (var line in Lines)
            {
                ...
                if (applyPromoCode && !string.IsNullOrEmpty(PromoCode))
                {
                    ...
                    // Fixed: Using proper rounding instead of truncating
                    linePrice = Math.Round(linePrice, 2);
                }
        ...
        {
            // Fixed: Using decimal instead of float for comparison to maintain precision
            return totalPrice < 500.0m;
        }
